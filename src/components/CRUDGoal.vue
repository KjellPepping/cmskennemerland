<template>
  <v-data-table :headers="headers" :items="goals" class="elevation-1">
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title class="font-weight-black">GOAL</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="success" dark class="mb-2" v-bind="attrs" v-on="on">New Goal</v-btn>
          </template>
              ...
              <v-container>
                <v-row>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedItem.goalId" label="Goal ID" type="number"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedItem.descirption" label="Description"></v-text-field>
                  </v-col>
                  ...
                  <v-dialog ref="dialog1" v-model="modal1" :return-value.sync="editedItem.endDate" persistent width="290px">
                   ...
                   <v-col cols="12" sm="6" md="4">
                    <v-select v-model="editedItem.creatorId" :items="users" item-text="name" item-value="userId" label="Creator"></v-select>
                  </v-col>
                  
                  <v-col cols="12" sm="6" md="4">
                     <v-select v-model="e6" :items="teams" item-text="name" item-value="teamId" :menu-props="{ maxHeight: '400' }" label="Relevant Teams" multiple  persistent-hint></v-select>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
  ...
  export default {
    data: () => ({
      ...
      [
        ...
        {text:"Description", value:"descirption"},
        {text:"Created On", value:"createdOn"},
        {text:"Edited On", value:"editedOn"},
        {text:"Start Date", value:"startDate"},
        {text:"End Date", value:"endDate"},
        {text:"Creator ID", value:"creatorId"},
        {text:"Relevant Teams", value:"relevantTeams"},
        {text:"", value:"actions"},
      ],
      goals:[],
      users:[],
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          axios.put('http://localhost:7071/api/goal',{ 
            ...
            creatorId:this.editedItem.creatorId,
            relevantTeams:this.editedItem.relevantTeams})
